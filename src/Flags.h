...
public:
	...
	Flags<T> operator |(T flag) const {return Flags<T>(m_value | static_cast<StorageType>(m_value));}
	Flags<T> operator |(const Flags<T>& flags) const {return Flags<T>(m_value | flags.m_value);}
	Flags<T>& operator |=(T flag) {m_value |= static_cast<StorageType>(m_value); return *this;}
